...
BOOL CAudioCD::Open( char Drive )
{
	...
	for ( ULONG i=Table.FirstTrack-1; i<Table.LastTrack; i++ )
	{
		...
		//Make this as "if track does not equal zero" or some bullshit. Also make a bool varible that denotes the existance of a track 0 and have it run if set to false. Since this is for doing the zero track, I may just have it return false or something
		if (TrackZero == false)
		{
